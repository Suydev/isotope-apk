...
| Auth-D0Y8CB1f | `if(s){z("/dashboard",{replace:!0});return e.jsx("div",{className:"min-h-screen",children:null});}const i=z()` | **signed-in users bounce off /auth** via the real `useNavigate` (z); `X` here is a lucide icon, NOT a router component — do not use `I.jsx(X,{to})` (login form only for signed-out; asserted in `test/auth-guard.test.mjs`) |
