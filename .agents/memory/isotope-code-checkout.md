...
## Real bug this uncovered: missing KaTeX fonts
`isotope-code/public/assets/vendor-katex-*.css` references KaTeX_Size3, KaTeX_Size4, and KaTeX_Typewriter font files (woff/woff2/ttf, 8 files total) that do not exist anywhere in `public/assets/` at HEAD. This is not a Replit-environment limitation — confirmed absent in the actual git tree.

**Why this matters:** `scripts/prepare-www.js` Step 6b hard-fails (exit 1) when these fonts are missing, which cascades to fail ~10 unrelated downstream tests (Floating Timer bridge packaging, restore-and-launch patching, Auth login patch, etc.) that never got a chance to run because the script aborted early. When debugging APK build test failures, always check Step 6b's output first before assuming the failures are independent bugs.

**How to apply:** This must be fixed upstream in `isotope-code` (the KaTeX font files need to be added to `public/assets/` and the CSS hash references reconciled), per the project's "fix isotope-code first" convention — isotope-apk should not patch around a missing source asset.
